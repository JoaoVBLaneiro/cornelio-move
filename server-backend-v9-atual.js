...
async function enviarPushExpo(expoPushToken, titulo, corpo, dados = {}) {
  if (!expoPushToken) {
    return;
  }
  ...
  if (
    ...
    console.log("Token push inválido ignorado:", token.slice(0, 30));
    return;
  }
  ...
  try {
    ...
    console.log("Push Expo enviado:", {
      status: resposta.status,
      retorno,
    });
  } catch (error) {
    console.log("Falha ao enviar push Expo:", error.message);
  }
}
...
io.on("connection", (socket) => {
  ...
  socket.on("motorista_online", (dados, callback) => {
    ...
    if (dados.expoPushToken) {
      pushTokensMotoristas.set(socket.id, String(dados.expoPushToken));
      console.log("Token push registrado para motorista:", {
